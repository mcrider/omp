...
class WorkflowSubmissionAssignmentPolicy extends AuthorizationPolicy {
	...
	function effect() {
		...
		$stageId = $monographStageAssignmentDao->getCurrentStageIdByMonographId($monograph->getId());
		...
		foreach($userGroupDao->getByRoleId($press->getId(), ROLE_ID_PRESS_ASSISTANT) as $userGroup) {
			if($monographStageAssignmentDao->assignmentExists($monographId, $stageId, $userId, $userGroup->getId())) {
				return AUTHORIZATION_PERMIT;
			}
		}
