...
/**
 * @file controllers/grid/files/galley/GalleyFilesGridRow.inc.php
 *
 * Copyright (c) 2003-2010 John Willinsky
 * Distributed under the GNU GPL v2. For full terms see the file docs/COPYING.
 *
 * @class GalleyFilesGridRow
...
class GalleyFilesGridRow extends GridRow {
	/**
	 * Constructor
	 */
	function GalleyFilesGridRow() {
		parent::GridRow();
	}
	...
	function initialize(&$request) {
		...
		if (!empty($rowId) && is_numeric($rowId)) {
			...
			// If there is no file uploaded, allow the user to upload if it is their signoff (i.e. their galley assignment)
			...
			// If there is a file uploaded, allow the user to edit it if it is their signoff (i.e. their galley assignment)
			if($copyeditedFileId && $signoff->getUserid() == $user->getId()) {
				$this->addAction(
					new LegacyLinkAction(
						'editGalleyFile',
						LINK_ACTION_MODE_MODAL,
						LINK_ACTION_TYPE_REPLACE,
						$router->url($request, null, null, 'editGalleyFile', null, $actionArgs),
						'common.edit',
						null,
						'add'
					));
