...
/**
 * @file controllers/grid/files/editorReviewFileSelection/FinalDraftFilesGridHandler.inc.php
 *
 ...
 *
 * @brief Handle the final draft files grid (displays files sent to editorial from the review stage)
 */
...
class FinalDraftFilesGridHandler extends GridHandler {
	...
	function initialize(&$request) {
		...
		$selectedFiles =& $reviewAssignmentDAO->getFinalDraftFilesByRound($monographId);
		...
		if ( $isSelectable ) {
			...
			$selectedRevisions =& $reviewAssignmentDAO->getFinalDraftFilesAndRevisionsByRound($monographId, $round, true);
		...
		if ($canAdd) {
			$this->addAction(
				new LinkAction(
					...
					'editor.submissionArchive.manageFinalDraftFiles',
					null,
					'add'
				)
			);
		...
		if ($canUpload) {
			$this->addAction(
				new LinkAction(
					...
					$router->url($request, null, 'grid.files.submissionFiles.SubmissionFinalDraftFilesGridHandler', 'addFile', null, array('monographId' => $monographId)),
