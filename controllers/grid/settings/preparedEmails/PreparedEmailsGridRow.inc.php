	...
	function initialize(&$request) {
		...
		if (isset($rowId) && is_string($rowId)) {
			...
			if (isset($emailTemplate) && $emailTemplate->isCustomTemplate()) {
				import('controllers.api.preparedEmails.linkAction.DeleteEmailLinkAction');
				$this->addAction(new DeleteEmailLinkAction($request, $rowId));
			} else {
				if($emailTemplate->getCanDisable()) {
					if($emailTemplate->getEnabled()) {
						import('controllers.api.preparedEmails.linkAction.DisableEmailLinkAction');
						$this->addAction(new DisableEmailLinkAction($request, $rowId));
					} else {
						import('controllers.api.preparedEmails.linkAction.EnableEmailLinkAction');
						$this->addAction(new EnableEmailLinkAction($request, $rowId));
					}
				}
			}
			...
			if (isset($emailTemplate) && !$emailTemplate->isCustomTemplate()) {
				import('controllers.api.preparedEmails.linkAction.ResetEmailLinkAction');
				$this->addAction(new ResetEmailLinkAction($request, $rowId));
			}
		}
	}
}