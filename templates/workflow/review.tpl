...
{url|assign:reviewFileSelectionGridUrl router=$smarty.const.ROUTE_COMPONENT component="grid.files.review.EditorReviewFilesGridHandler" op="fetchGrid" stageId=$smarty.const.WORKFLOW_STAGE_ID_INTERNAL_REVIEW monographId=$monograph->getId() reviewType=$currentReviewType round=$selectedRound canAdd=1 escape=false}
...
{url|assign:revisionsGridUrl router=$smarty.const.ROUTE_COMPONENT component="grid.files.submission.RevisionsGridHandler" op="fetchGrid" monographId=$monograph->getId() reviewType=$currentReviewType round=$selectedRound escape=false}
